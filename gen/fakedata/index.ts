...
const mapStrategy = (pos: any[], mod: any[]): StrategyItem[] => {
    ...
    return Object.keys(gData).map(k => {
        ...
        return {
            ...
            radar: items[0].radar,
            currentWeight: sumBy(items, w => w.currentWeight),
            currentQuantity: sumBy(items, w => w.currentQuantity),
            currentPrice: items[0].currentPrice,
            currentAmount: sumBy(items, w => w.currentAmount),
            modelWeight: sumBy(items, w => w.modelWeight),
            suggestedDelta: 0,
            suggestionAccepted: false,
