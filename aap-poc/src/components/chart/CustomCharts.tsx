...
const { ComposedChart, Legend, ResponsiveContainer, Sector, PieChart, Pie, Cell, Bar, XAxis, YAxis, CartesianGrid , BarChart,Tooltip} = require('recharts');
...
const ColorsDict = {
  ...
  Currency: Colors.YELLOW,
  Rating: Colors.ACQUA,
  MacroAssetClass: Colors.GRAY,
  MicroAssetClass: Colors.VIOLET
}
...
export class CustomBarChart extends React.Component<CustomChartProps, CustomChartState> {
/*
    this.renderCustomizedLabel = this.renderCustomizedLabel.bind(this);
  /*  renderCustomizedLabel = (props: any) => {
        const { x, y, width, height, value } = props;
    ...
    return (
      <CustomResponsiveContainer height={responsiveHeight || height}>
        <BarChart width={600} height={300} data={data}
          ...
          <YAxis />
          <Tooltip />
          <Bar dataKey={dataKey} name={nameKey} fill={color} />
        </BarChart>
      </CustomResponsiveContainer>
    );
  }
