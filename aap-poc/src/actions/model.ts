...
export const filterMapItems: { [key in FilterMapTypes]: FilterMap } = {
  ...
  Alerts: {
    ...
    subprop: 'numOfAlerts',
    searchprop: 'alerts',
    render: { header: 'Alerts', icon: 'alarm', label: (value: number) => `${value} alert${value > 1 ? 's' : ''}` }
  },
  Agents: {
    prop: 'agent',
    searchprop: 'agents',
  ...
  AlertType: {
    ...
    render: { header: 'Alert Type', icon: 'alarm', label: (value: string) => startCase(value) }
  },
  Segment: {
    prop: 'segment',
    searchprop: 'segments',
    ...
    data.forEach(client => {
      ...
      if (isArray(currentValues)) {
        currentValues.forEach((v, i) => {
          const pre = filter[v];
          filter = { ...filter, [v]: { ...pre, current: (pre ? (pre.current || 0) : 0) + 1 } }
        });
      }
    });
