...
export const getSuggestedTransactions = (args: { holdings: Model.Holding[], proposed: Model.RadarItem } | undefined) => {
  ...
  const sell = numArray(nBuy).map(y => {
...
export const getBreakdown = (holdings: Model.Holding[] | undefined) => {
  const attributes = [
    'MacroAssetClass',
    'MicroAssetClass',
    'Sector',
...
export const getStrategy = (clientId: string | undefined) => {
  return Promise.resolve(
    strategies[clientId!]
...
export const getSuggestion = (args: { id: string, position: Model.StrategyItem[], axes: Model.RadarStrategyParm, calculateFromAxes: boolean } | undefined) => {
  if (!args) return Promise.reject();
...
} | undefined): Promise<Model.Holding[]> => {
